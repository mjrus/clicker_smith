...
class Menu:
    ...
    def __init__(self, width, height, has_save):
        ...
        y: int = self.height * .5
        self.options = [
            MenuOptions(x, y + 75, 300, 50, "New Game"),
            MenuOptions(x, y + 150, 300, 50, "Settings"),
            MenuOptions(x, y + 225, 300, 50, "Exit")
        ]
        if has_save:
            self.options.append(MenuOptions(x, y, 300, 50, "Continue"))

    def render(self, surface):
        surface.fill((42, 55, 61))
        menu_tile = font.SysFont("verdana", 64)
        menu_background = draw.rect(surface, (67, 87, 96),
                                    Rect(round(self.width * 0.3125), 0, self.width * 0.375, self.height))
        clicker = menu_tile.render("Clicker", True, (255, 255, 255))
        smith = menu_tile.render("Smith", True, (255, 255, 255))
        surface.blit(clicker,
                     (menu_background.x + ((menu_background.width / 2) - clicker.get_width() / 2), self.height * 0.05))
        surface.blit(smith, (menu_background.x + ((menu_background.width / 2) - smith.get_width() / 2),
                             self.height * 0.05 + clicker.get_height()))
...
class MenuOptions:
    ...
    def render(self, surface):
        ...
        surface.blit(title, (button_background.x + (button_background.width - title.get_width()) / 2,
                             button_background.y + (button_background.height - title.get_height()) / 2))
